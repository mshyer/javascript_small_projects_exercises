...
function displayableDateRange(emailDates) {
  let dateParts = emailDates.map(str => new Date(str.split(' ')[1]));
  let minMax = minMaxDates(dateParts);
  return `${minMax[0].toDateString()} - ${minMax[1].toDateString()}`;
}

function minMaxDates(dateParts) {
  dateParts.sort((d1, d2) => d1.getTime() - d2.getTime());
  let min = dateParts[0];
  let max = dateParts[dateParts.length - 1];
  return [min, max];
}
