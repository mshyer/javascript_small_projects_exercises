// Input: String containing multiple email messages  separated by ##||##
//    Email message has five parts, delimiter is #/#
//      -sender, subject, date, recipient, body
//      -occur in the predicted sequence
//
...
function mailCount(emailData) {
  ...
  const EMAILS_COUNT = emails.length;
  //emails.forEach(email => console.log(email));
  let regx = /Date.*#\/#/g;
  let dateMatches = emailData.match(regx).map(
    str => new Date(str.slice(6, 16))
  );
  dateMatches.sort((d1, d2) => d1.getTime() - d2.getTime());
  console.log(`Count of email:  ${EMAILS_COUNT}`);
  console.log(`Range: ${dateMatches[0].toDateString()} - \
${dateMatches[dateMatches.length - 1].toDateString()}`);
